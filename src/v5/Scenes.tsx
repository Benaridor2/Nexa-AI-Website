...
const chatRenderer = (root: HTMLElement) => {
  ...
  return (p: number) => {
    const open = phase(p, .025, .26), send = phase(p, .43, .54), focus = between(p, .78, .88, .95, 1);
    styles(stage, { '--portal-open': open });
    shutters.forEach((el,i) => styles(el, { transform: `translateX(${(i ? 1 : -1)*open*110}%) rotateY(${(i ? 1 : -1)*open*35}deg)`, opacity: 1-phase(p,.16,.28) }));
    ...
    visible(welcome,1-phase(p,.39,.45));
    visible(tools,1-phase(p,.43,.48),false);
    styles(tools,{height:`${36*(1-phase(p,.48,.54))}px`,'margin-top':`${12*(1-phase(p,.48,.54))}px`});
    chunks.forEach((el,i)=>visible(el,phase(p,.28+i*.024,.31+i*.024),false));
    visible(dock,phase(p,.48,.54),false);
    visible(search,between(p,.54,.57,.61,.64));
    visible(response,phase(p,.63,.66));
    lines.forEach((el,i)=>{const t=phase(p,.63+i*.027,.68+i*.027);visible(el,t);styles(el,{transform:`translateY(${22*(1-t)}px)`});});
    styles(photo,{'clip-path':`inset(${(1-phase(p,.70,.79))*100}% 0 0 0 round 10px)`});
  ...
  return <section id="guest-story" className="conversation scene-section" ref={ref} aria-labelledby="guest-title">
    <div className="scene-stage wrap" data-animated>
      ...
      <div className="chat-window" data-animated>
        ...
        <div className="chat-dock" data-animated aria-hidden="true"><span>Ask ChatGPT</span><ComposerTools /></div>
      </div>
...
const journeyRenderer = (root: HTMLElement) => {
  ...
  return (p: number) => {
    ...
    visible(q('.site-ui'), phase(p, .47, .53));
    styles(q('.site-ui'), { 'clip-path': `inset(0 ${(1-phase(p,.47,.61))*100}% 0 0 round 12px)` });
