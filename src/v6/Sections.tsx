...
function AnswerMock({ priced }: { priced: boolean }) {
  ...
  return <div className={`answer-mock ${priced ? 'is-priced' : 'is-unpriced'}`} aria-hidden="true">
    <div className="mock-card">
      <div className="mock-head"><span className="mock-orb"/><b>Your property</b><small>{priced ? 'Verified' : 'Not verified'}</small></div>
...
export function PricedOrUnpriced() {
  return <section className="duo" id="priced" aria-labelledby="priced-title">
    <div className="wrap">
      <Label>[01] THE TWO WORDS / PRICED OR UNPRICED</Label>
      <header className="duo-head" data-rise>
      ...
      <div className="duo-grid">
        <article className="duo-card is-unpriced" data-rise>
        ...
        <article className="duo-card is-priced" data-rise>
          <AnswerMock priced/>
      ...
      <div className="duo-close" data-rise>
...
export function HowItWorksSteps() {
  return <section className="steps section-pad" id="how-it-works" aria-labelledby="works-title">
    <div className="wrap">
      <Label>[02] HOW IT WORKS / THE FIX</Label>
      <header className="steps-head" data-rise><h2 id="works-title">NEXA AI makes your property PRICED.<br/><em>Here is how.</em></h2><p>From "find me a place" to a booking on your site. One conversation.</p></header>
      <div className="steps-flow">
        <div className="flow-rail" aria-hidden="true"><span/><span/><span/><span/><i className="flow-packet"/></div>
        <ol>{STEPS.map(([title, text], i) => <li key={title} style={{ '--i': i } as React.CSSProperties} data-rise><StepVisual step={i}/><span className="step-number">0{i + 1}</span><h3>{title}</h3><p>{text}</p></li>)}</ol>
      </div>
      <div className="steps-foot" data-rise>
...
export function ProductConnector() {
  return <section className="connector section-pad" id="connector" aria-labelledby="connector-title">
    <div className="wrap">
      <Label>[03] THE PRODUCT / NEXA AI CONNECTOR</Label>
      <header className="split-heading" data-rise><div><h2 id="connector-title">One connection.<br/><em>Two kinds of guests.</em></h2></div><p>Both book direct. You do not choose between them. One connection answers both.</p></header>
      <div className="product-flow" data-rise>
