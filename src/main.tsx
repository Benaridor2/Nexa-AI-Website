...
const page = isV1
  ...
  : isV2
    ...
    : Promise.all([import('./v7/App'), import('./v6/style.css'), import('./v7/style.css')]);

void page.then(([{ default: App }]) => {
  createRoot(document.getElementById('root')!).render(<React.StrictMode>
    <App />
    {(isV1 || isV2 || isV3 || isV4 || isV5 || isV6) && <a href="/" style={{ position: 'fixed', right: 20, bottom: 20, zIndex: 100, background: '#863db3', color: 'white', padding: '13px 20px', borderRadius: 5, fontSize: 13 }}>View latest ↗</a>}
  </React.StrictMode>);
});
