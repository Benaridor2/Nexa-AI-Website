...
export function useScene(ref: RefObject<HTMLElement | null>, enabled: boolean, setup: Renderer) {
  useLayoutEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      trigger: root, start: 'top top', end: 'bottom bottom', animation: tl,
    ...
    return () => {
      ...
      root.querySelectorAll<HTMLElement>('[data-animated]').forEach(el => {
        el.removeAttribute('style'); el.removeAttribute('aria-hidden'); el.removeAttribute('inert');
      });
