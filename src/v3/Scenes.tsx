...
const chatRenderer = (root: HTMLElement) => {
  ...
  return (p: number) => {
    ...
    styles(frame, { transform: `translateY(${24 * (1 - phase(p, 0, .12)) - 7 * focus}px) scale(${.94 + .06 * phase(p, 0, .12) + .025 * focus + composeFocus})` });
...
export function Conversation({ motion }: { motion: boolean }) {
  ...
  return <section id="guest-story" className="conversation scene-section" ref={ref} aria-labelledby="guest-title">
    <div className="scene-stage wrap">
      <header className="scene-heading centered"><Label>01 / THE GUEST EXPERIENCE</Label><h2 id="guest-title">Your next guest <em>starts here.</em></h2><p>A normal question. A direct way to book.</p></header>
      <div className="chat-window" data-animated>
        ...
        <div className="chat-app-header"><span>ChatGPT <span className="chevron">⌄</span></span><span className="chat-header-actions" aria-hidden="true">↗ &nbsp; ···</span></div>
        ...
        <div className="chat-response" data-animated>
          ...
          <div className="answer-beat answer-facts" data-animated><span>{STAY.shortDates} · {STAY.guests} · {STAY.nights}</span><strong>{STAY.total} <span>total</span></strong></div>
          <a className="answer-beat source-link" data-animated href={STAY.url} target="_blank" rel="noreferrer">View and book on Sea N' Rent <Arrow diagonal /></a>
        </div>
...
export function Comparison({ motion }: { motion: boolean }) {
  ...
  return <section className="comparison scene-section" id="priced" ref={ref} aria-labelledby="priced-title"><div className="scene-stage wrap">
    <div className="comparison-copy"><Label>02 / THE DIFFERENCE</Label><h2 id="priced-title">A mention becomes<br/> a <em>bookable answer.</em></h2><p>Give the answer the three things a guest needs to take the next step.</p><div className="price-word-wrap"><span className="unpriced-word" data-animated>UNPRICED</span><span className="priced-word" data-animated>PRICED<span>↗</span></span></div></div>
    <div className="comparison-card"><div className="comparison-property"><Photo name="living"/><div><small>TEL AVIV · SEA N' RENT</small><h3>{STAY.property}</h3></div></div>
      ...
      <p className="comparison-note">An illustrative before and after, not an assessment of this property's current AI visibility.</p>
    </div>
  </div></section>;
}
...
export function BookingJourney({ motion }: { motion: boolean }) {
  ...
  return <section className="booking-journey scene-section" id="how-it-works" ref={ref} aria-labelledby="works-title"><div className="scene-stage wrap">
    <header className="scene-heading centered"><Label>03 / HOW IT WORKS</Label><h2 id="works-title">From their question <em>to your checkout.</em></h2></header>
